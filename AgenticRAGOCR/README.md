...
### v1.2.0 (最新) - 语义连贯性与空间感知增强版
在 v1.1.0 基础上，进一步解决了 RAG 全文理解弱及局部上下文断裂的问题：
- **全局语义增强 (Global Context)**：
  - **自动摘要索引**：上传文档时自动生成全文摘要并建立“全局总结索引”，解决“问 A 回 B”或无法回答宏观问题的问题。
  - **主旨注入**：在每个局部块的向量表示中注入全局摘要信息，让局部块具备“全局视野”。
- **空间临近性感知 (Spatial Proximity)**：
  - **滑动窗口扩展**：检索命中某块时，自动拉取其在物理页面上的“前文”与“后文”块，形成连续的语义窗口。
  - **顺序索引 (Seq-Index)**：引入页内绝对顺序索引，确保跨块的逻辑推导（如“紧接上文...”）能够被准确理解。
- **稳定性修复**：
  - **端口冲突解决**：统一前后端默认端口为 `8100`，修复 `net::ERR_CONNECTION_REFUSED` 连接报错。
  - **配置标准化**：规范化 `.env` 与 `config.py` 的加载逻辑，提升部署稳定性。

### v1.1.0 - RAG 深度优化版
针对原始版本检索不到、回答发散等问题进行了深度加固：
- **分块逻辑优化**：
  - **父子块架构**：子块（400字符）用于精准向量匹配，父块（完整块）用于 LLM 上下文，兼顾检索精度与语义完整性。
  - **10% 重叠度**：恢复 40 字符重叠，解决边界语义截断问题。
- **OCR 增强处理**：
  - **空间+结构合并**：新增基于 Y 轴距离（<1.5倍行高）与标点符号启发式的块合并逻辑，解决 OCR 碎片化导致的语义断裂。
- **LLM 回答加固**：
  - **严谨模式 Prompt**：强制要求“严禁发散猜测”、“引用优先”，如果文档无相关信息必须回答“未提及”。
  - **参数调优**：将 `temperature` 降低至 `0.01`，关闭模型创造力，确保输出高度稳定且忠实于原文档。
- **上下文工程**：在向量索引中注入文件路径、页码、块类型等元数据前缀，增强语义检索深度。

### v1.0.0 - 初始版本
- 基础多模态文档解析（PaddleOCR/PyMuPDF）
- 基础 RAG 检索问答
- 前后端完整链路实现

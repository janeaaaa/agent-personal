...
# 强制加载 .env 并覆盖环境变量，确保 API Key 正确
...
async def get_indexing_progress(doc_id: str):
    ...
    try:
        ...
        # 兼容处理：如果是列表（新版全量结果），或者是字典（旧版单页结果）
        blocks = []
        if isinstance(ocr_data, list):
            # 新版全量结果，每个元素是一个页面的 OCRResult
            for page_res in ocr_data:
...
async def get_document_blocks(
    ...
    try:
        ...
        # 优先使用 full_res.json 如果存在
        full_res_file = next((f for f in json_files if f.name == "full_res.json"), None)
        if full_res_file:
            json_files = [full_res_file]
        ...
        for file_index, json_file in enumerate(json_files):
            ...
            # 兼容处理：如果是列表（新版全量结果），或者是字典（旧版单页结果）
            page_blocks = []
            page_index = file_index
            
            if isinstance(ocr_data, list):
                # 新版全量结果，包含所有页面，我们根据 file_index 找对应页或者合并
                # 如果是 full_res.json，它已经包含了所有页
                if json_file.name == "full_res.json":
                    for p_res in ocr_data:
                        p_idx = p_res.get('page_index', 0)
                        if page is not None and p_idx != page:
                            continue
                        for b in p_res.get('blocks', []):
                            b['page_index'] = p_idx
                            b['source_file'] = json_file.name
                            page_blocks.append(b)
                else:
                    # 其他列表格式，尝试取第一个
                    if ocr_data:
                        page_blocks = ocr_data[0].get('blocks', [])
                        page_index = ocr_data[0].get('page_index', file_index)
            else:
                # 旧版单页结果
                page_blocks = ocr_data.get('parsing_res_list', [])
                page_index = ocr_data.get('page_index', file_index)
